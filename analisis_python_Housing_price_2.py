...
df['PriceCategory'] = df['price'].apply(
    lambda x: 'Cara' if x > price_median else 'Barata')

print(f"Dimensiones del dataset: {df.shape}")
print(f"Variable objetivo creada: PriceCategory")
print(
    f"   - Caras (>${price_median:,.0f}): {sum(df['PriceCategory'] == 'Cara')} propiedades")
print(
    f"   - Baratas (≤${price_median:,.0f}): {sum(df['PriceCategory'] == 'Barata')} propiedades")
...
for name, config in models.items():
    ...
    auc_score = roc_auc_score(
        y_test, y_pred_proba) if y_pred_proba is not None else None
    ...
    if name == 'Arbol de Decision':
        tree_rules['reglas_texto'] = export_text(
            best_model, feature_names=list(X.columns))
        tree_rules['caracteristicas_importancia'] = dict(
...
plt.title('Accuracy en Validacion Cruzada por Algoritmo',
...
bars = plt.bar(algorithms, times, color=[
...
bars = plt.bar(algorithms, f1_scores, color=[
               '#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd', '#8c564b'])
...
plt.subplot(2, 2, 4)
auc_scores = [results[name]['auc'] if results[name]
              ['auc'] is not None else 0 for name in algorithms]
bars = plt.bar(algorithms, auc_scores, color=[
               '#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd', '#8c564b'])
...
plt.title('Curvas ROC - Comparacion de Algoritmos',
...
plot_tree(best_tree_model,
          ...
          impurity=False,
          proportion=True)
plt.title('Arbol de Decision - Visualizacion Completa',
...
if hasattr(lr_model, 'coef_'):
    ...
    plt.barh(coefficients['Variable'],
             coefficients['Coeficiente'], color='darkorange')
    plt.xlabel('Valor del Coeficiente')
    plt.title('Coeficientes de Regresion Logistica',
              fontsize=16, fontweight='bold')
    plt.axvline(x=0, color='black', linestyle='-', alpha=0.8)
    plt.grid(axis='x', alpha=0.3)
    plt.tight_layout()
    plt.savefig('regresion_logistica_coeficientes.png',
...
metrics_comparison.to_csv(
    'metricas_comparacion_modelos.csv', index=False, encoding='utf-8')
print("Archivo guardado: metricas_comparacion_modelos.csv")

# 5.2 Guardar datos del arbol de decision
if tree_rules:
    ...
    importancia_df.to_csv(
        'arbol_decision_importancia_variables.csv', index=False, encoding='utf-8')
    ...
    # Predicciones del arbol de decision
    ...
    predicciones_arbol.to_csv(
        'arbol_decision_predicciones.csv', index=False, encoding='utf-8')
...
if hasattr(lr_model, 'coef_'):
    ...
    coefficients.to_csv('regresion_logistica_coeficientes.csv',
...
matriz_confusion_df.to_csv(
    'mejor_modelo_matriz_confusion.csv', encoding='utf-8')
...
print(
    f"   AUC: {best_result['auc']:.4f}" if best_result['auc'] else "   AUC: N/A")
print(f"   Parametros: {best_result['best_params']}")

print(f"\nMETRICAS EN TEST DE TODOS LOS MODELOS:")
for name, result in results.items():
    print(
        f"   {name:<20}: F1 = {result['f1']:.4f}, Acc = {result['accuracy']:.4f}")

print(f"\nINFORMACION TECNICA:")
print(
    f"   Tiempo total entrenamiento: {sum(training_times.values()):.2f} segundos")
print(
    f"   Algoritmo mas rapido: {min(training_times, key=training_times.get)}")
